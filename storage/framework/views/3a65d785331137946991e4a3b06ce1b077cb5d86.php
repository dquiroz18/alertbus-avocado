	      	...
	      	<form>
	      		...
	      		<div class="row">
		            ...
		            <div class="form-group col-sm-4">
		              ...
		              <input type="date" class="form-control" id="txtDesde" name="desde" required>
		            </div>
		            <div class="form-group col-sm-4">
...
<?php $__env->startSection('script'); ?>
	...
	<script>
		$('#buscar').click(function() {
	  	...
	  	data = getModelByParams({idProveedor: o_id}, "<?php echo e(url('mantenimiento/tarifas/listar')); ?>", 'GET');
	  	listarOnTable(6, 8, data, [0, 2, 4, 6], true, true, false);
	  });

		$('#cmbTransportistaM').change(function() {
			var idProveedor = $(this).val();
			var rutas = getModelByParams({idProveedor: idProveedor, combo: 1}, "<?php echo e(url('mantenimiento/rutas/listar')); ?>", 'GET');
			var select = $('#cmbRuta');
			listarOnSelect(0, 1, rutas, select);
			var tipoVehiculos = getModelByParams({idProveedor: idProveedor}, "<?php echo e(url('mantenimiento/vehiculos/tipo-vehiculos')); ?>", 'GET');
			var select = $('#cmbTipoVehiculo');
			listarOnSelect(0, 3, tipoVehiculos, select);
			var tipoTarifas = getModelByParams({idProveedor: idProveedor}, "<?php echo e(url('mantenimiento/tarifas/tipo-tarifas')); ?>", 'GET');
		...
		$('#addTipoVehiculo').click(function() {
			...
			if (response = saveModel({idProveedor: idProveedor, tipoVehiculo: tipoVehiculo}, "<?php echo e(url('mantenimiento/vehiculos/tipo-vehiculo')); ?>", 'POST')){
		...
		$('#divTipoVehiculo').on('click', '.tpDelete', function(e) {
			...
			if (response = saveModel({tipoVehiculo: tipoVehiculo, idProveedor: idProveedor}, "<?php echo e(url('mantenimiento/vehiculos/tipo-vehiculo/delete')); ?>", 'POST')) {
				var message = response.message;
				var success = response.success;
				alert(message);
				if (success == '0') {
		...
		$('#addTipoTarifa').click(function() {
			...
			if (response = saveModel({idProveedor: idProveedor, tipoTarifa: tipoTarifa}, "<?php echo e(url('mantenimiento/tarifas/tipo-tarifa')); ?>", 'POST')){
				var message = response.message;
				var success = response.success;
				alert(message);
				if (success == '0') {
		...
		$('#divTipoTarifa').on('click', '.tpDelete', function(e) {
			...
			var idProveedor = $(this).data('proveedor');
			if (response = saveModel({tipoTarifa: tipoTarifa, idProveedor: idProveedor}, "<?php echo e(url('mantenimiento/tarifas/tipo-tarifa/delete')); ?>", 'POST')) {
				var message = response.message;
				var success = response.success;
				alert(message);
				if (success == '0') {
		...
		$('#new').on('show.bs.modal', function() {
			if (id != 0) {
				var model = getModelByParams({idTarifa: id}, "<?php echo e(url('mantenimiento/tarifa')); ?>", 'GET');
				...
				$('#txtDesde').val(model.desde.substring(0, 10));
		...
		$('#new form').submit(function(e) {
		  	...
		  	//console.log(data);
		  	if(response = saveModel(data, "<?php echo e(url('mantenimiento/tarifa')); ?>", 'POST')){
		  		var message = response.message;
		  		var success = response.success;
	  ...
	  $('#delete form').submit(function(e) {
	  	...
	  	var data = $(this).serialize();
	  	//console.log(data);
	  	if(response = saveModel(data, "<?php echo e(url('mantenimiento/tarifa/delete')); ?>", 'POST')){
	  		var message = response.message;
	  		var success = response.success;
	  		showMessage(success, message);
	  		$('#buscar').trigger('click');
