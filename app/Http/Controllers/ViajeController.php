    ...
    public function subir(Request $request)
    {
        ...
        Excel::selectSheetsByIndex(0)->load($file, function($reader) use ($string) {
            ...
            foreach ($results as $row) {
                ...
                $sql = DB::select("exec [SP_AlertBus_Programacion_Importar] ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?", [$fecha, $hora, $row->empresa, $row->area, $row->centrocosto, $row->transportista, $row->ruta, $row->tipovehiculo, $row->tarifa, $row->tarifafinal , $row->conductor, $row->vehiculo,  Auth::user()->usuario]);
