...
@section('script')
	...
	<script>
		...
		$('#buscar').click(function() {
		  	...
		  	data = getModelByParams({idProveedor: o_id}, "{{ url('mantenimiento/rutas/listar') }}", 'GET');
		...
		$('#cmbTransportistaM').change(function() {
			...
			origenes = getModelByParams({idTransportista: id}, "{{ url('mantenimiento/rutas/origenes') }}", 'GET');
			select = $('#txtOrigen');
		  	listarOnSelect(0, 0, origenes, select);
		  	destinos = getModelByParams({idTransportista: id}, "{{ url('mantenimiento/rutas/destinos') }}", 'GET');
			select = $('#txtDestino');
		...
		$('#new').on('show.bs.modal', function() {
			if (id != 0) {
				var model = getModelByParams({idRuta: id}, "{{ url('mantenimiento/ruta') }}", 'GET');
		...
		$('#new form').submit(function(e) {
		  	...
		  	//console.log(data);
		  	if(response = saveModel(data, "{{ url('mantenimiento/ruta') }}", 'POST')){
	  ...
	  $('#delete form').submit(function(e) {
	  	...
	  	var data = $(this).serialize();
	  	//console.log(data);
	  	if(response = saveModel(data, "{{ url('mantenimiento/ruta/delete') }}", 'POST')){
	  		var message = response.message;
	  		var success = response.success;
	  		showMessage(success, message);
	  		$('#buscar').trigger('click');
